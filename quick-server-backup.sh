...
docker exec servaan-postgres-server pg_dump -U servaan -d servaan_prod -Fc -f "/tmp/${BACKUP_NAME}.dump"

# Copy to host
docker cp "servaan-postgres-server:/tmp/${BACKUP_NAME}.dump" "$BACKUP_DIR/"
