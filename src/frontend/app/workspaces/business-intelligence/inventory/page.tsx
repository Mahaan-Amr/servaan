...
export default function InventoryDashboardPage() {
  ...
  const fetchBIData = useCallback(async () => {
    try {
      ...
      const dashboardResponse = await biService.getDashboard(period, undefined, undefined, 'inventory');
      ...
      if (dashboardKPIs?.activeProductsCount) {
        quickReports.push({
          id: '3',
          title: 'محصولات فعال',
          description: 'تعداد کل محصولات فعال',
          value: `${dashboardKPIs.activeProductsCount || 0}`,
          change: 0,
          trend: 'neutral'
        });
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-4">در حال بارگذاری داشبورد موجودی...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <Section className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
              داشبورد موجودی (Inventory)
            </h1>
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-1">
              تحلیل و گزارش‌گیری از سیستم موجودی
            </p>
          </div>
      ...
      {/* Charts Section */}
      {chartData && chartData.categoryChart && chartData.categoryChart.labels && chartData.categoryChart.labels.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 sm:p-6">
          ...
          <CustomPieChart
            ...
            height={300}
            enableExport={true}
            chartId="inventory-category-chart"
          />
        </div>
      )}
    </Section>
