...
export default function ItemsPage() {
  ...
  // Handle item deletion - open confirmation modal
  ...
  const confirmDeleteItem = async () => {
    ...
    try {
      setDeletingItem(pendingDeleteItem.id);
      setShowDeleteModal(false);
      
      await itemService.deleteItem(pendingDeleteItem.id);
      
      toast.success(`کالای "${pendingDeleteItem.name}" با موفقیت حذف شد`);
  ...
  // Handle stock reset - open confirmation modal
  const handleResetStock = (itemId: string, itemName: string, currentStock: number) => {
    if (currentStock === 0) {
      toast.error('موجودی این کالا در حال حاضر صفر است');
      return;
    }

    setPendingResetItem({ id: itemId, name: itemName, currentStock });
    setShowResetModal(true);
  };

  // Confirm stock reset
  const confirmResetStock = async () => {
    if (!pendingResetItem) return;

    try {
      setResettingItem(pendingResetItem.id);
      setShowResetModal(false);
      
      await inventoryService.resetItemStock(pendingResetItem.id);
      
      toast.success(`موجودی کالای "${pendingResetItem.name}" با موفقیت به صفر بازنشانی شد`);
      
      // Refresh inventory status
      const updatedInventory = await inventoryService.getCurrentInventory();
      setInventoryStatus(updatedInventory);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'خطا در بازنشانی موجودی';
      toast.error(errorMessage);
    } finally {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
                    ...
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium">
                        <div className="flex flex-col sm:flex-row gap-1 sm:gap-3">
                          ...
                          <button
                            onClick={() => handleResetStock(item.id, item.name, currentStock)}
                            className="text-orange-600 hover:text-orange-900 dark:text-orange-400 dark:hover:text-orange-300"
                            disabled={resettingItem === item.id || currentStock === 0}
                            title="بازنشانی موجودی به صفر"
                          >
                            {resettingItem === item.id ? (
      ...
      <ConfirmationModal
        ...
        message={pendingDeleteItem ? `آیا از حذف کالای "${pendingDeleteItem.name}" اطمینان دارید؟\n\nاین عملیات غیرقابل بازگشت است.` : ''}
        confirmText="حذف"
        cancelText="انصراف"
        ...
        isLoading={deletingItem === pendingDeleteItem?.id}
      />
      ...
      <ConfirmationModal
        ...
        title="تأیید بازنشانی موجودی"
        message={pendingResetItem ? `آیا از بازنشانی موجودی کالای "${pendingResetItem.name}" به صفر اطمینان دارید؟\n\nموجودی فعلی: ${pendingResetItem.currentStock}\nاین عملیات یک تراکنش خروج ثبت می‌کند.` : ''}
        ...
        isLoading={resettingItem === pendingResetItem?.id}
      />
    </div>
  );
} 