  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white">بخش‌بندی مشتریان</h1>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
          <button
            ...
            className="inline-flex items-center justify-center px-3 py-2 sm:px-4 sm:py-2 border border-transparent text-xs sm:text-sm font-medium rounded-md shadow-sm text-white bg-pink-600 hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 disabled:opacity-50"
          >
            {updateLoading ? (
          ...
          <button
            ...
            className="inline-flex items-center justify-center px-3 py-2 sm:px-4 sm:py-2 border border-gray-300 dark:border-gray-600 text-xs sm:text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
          >
            <svg className="w-3 h-3 sm:w-4 sm:h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {Object.entries(segmentData.segmentDistribution).map(([segment, data]) => (
          <div key={segment} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
            <div className="flex items-center">
              <div className={`w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${segmentColors[segment as keyof typeof segmentColors]} rounded-lg flex items-center justify-center`}>
              ...
              <div className="mr-3 sm:mr-4">
                ...
                <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">{segment}</p>
              </div>
            </div>
      ...
      {segmentData.recentMovements && segmentData.recentMovements.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-gray-700">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">تغییرات اخیر بخش\u200cها</h3>
          </div>
      ...
      {segmentData.insights && segmentData.insights.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-gray-700">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">بینش\u200cهای بخش\u200cبندی</h3>
          </div>
          <div className="p-4 sm:p-6">
            <div className="space-y-3">
              {segmentData.insights.map((insight: string, index: number) => (
                <div key={index} className="flex items-start">
                  ...
                  <p className="text-sm sm:text-base text-gray-700 dark:text-gray-300">{insight}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {Object.entries(segmentData?.segmentDistribution || {}).map(([segment, data]: [string, { count: number; totalValue: number }]) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div className={`w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br ${segmentColors[segment]} rounded-lg flex items-center justify-center`}>
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
              </div>
              <div className="text-right">
                <div className="text-sm sm:text-base font-semibold text-gray-900 dark:text-white">{data.count}</div>
                <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">{segmentLabels[segment]}</div>
              </div>
            </div>
          </button>
        ))}
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      {upgradeableCustomers.length > 0 ? (
          ...
          <div className="p-4 sm:p-6">
            <div className="space-y-3 sm:space-y-4">
              {upgradeableCustomers.map((customer: CustomerSegmentData) => (
                <div key={customer.customerId} className="p-3 sm:p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                  ...
                  <div className="mt-3 sm:mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                    <div className="text-center">
                      ...
                      <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">بازدید</div>
                    </div>
                    <div className="text-center">
                      <div className="text-sm sm:text-lg font-semibold text-gray-900 dark:text-white">
                        {customer.loyaltyData.lifetimeSpent.toLocaleString('fa-IR')}
                      </div>
                      <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">کل خرید</div>
                    </div>
                    <div className="text-center">
                      <div className="text-sm sm:text-lg font-semibold text-gray-900 dark:text-white">
                        {customer.loyaltyData.currentYearSpent.toLocaleString('fa-IR')}
                      </div>
                      <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">خرید امسال</div>
                    </div>
                    <div className="text-center">
                      <div className="text-sm sm:text-lg font-semibold text-gray-900 dark:text-white">
                        {customer.loyaltyData.lastVisitDays}
                      </div>
                      <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">روز پیش</div>
                    </div>
                  </div>
                  ...
                  {customer.reasons && customer.reasons.length > 0 && (
                    <div className="mt-3 sm:mt-4">
                      <div className="text-xs sm:text-sm font-medium text-gray-900 dark:text-white mb-2">
                        دلایل ارتقا:
                      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 sm:p-12 text-center">
          <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-gray-400 to-gray-500 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto">
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="space-y-3 sm:space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm sm:text-base border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-pink-500 focus:border-pink-500 dark:bg-gray-700 dark:text-white"
          ...
          {/* Add Rules */}
          <div className="space-y-3 sm:space-y-4">
            <h4 className="text-base sm:text-lg font-medium text-gray-900 dark:text-white">قوانین بخش\u200cبندی</h4>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              <select
                value={currentRule.field}
                onChange={(e) => setCurrentRule((prev: SegmentRule) => ({ ...prev, field: e.target.value }))}
                className="px-3 py-2 text-sm sm:text-base border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-pink-500 focus:border-pink-500 dark:bg-gray-700 dark:text-white"
              >
              ...
              <button
                onClick={addRule}
                className="px-3 py-2 sm:px-4 sm:py-2 bg-pink-600 text-white text-xs sm:text-sm rounded-md hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-pink-500"
              >
                افزودن قانون
              </button>
            </div>
          ...
          {formData.rules.length > 0 && (
            <div className="space-y-2">
              <h5 className="text-sm sm:text-base font-medium text-gray-900 dark:text-white">قوانین تعریف شده:</h5>
              {formData.rules.map((rule: SegmentRule, index: number) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
                  ...
                  <button
                    ...
                    className="text-xs sm:text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-200"
                  >
                    حذف
                  </button>
                </div>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-end gap-3 sm:gap-4">
          <button
            onClick={onClose}
            className="px-3 py-2 sm:px-4 sm:py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 text-xs sm:text-sm rounded-md hover:bg-gray-50 dark:hover:bg-gray-700"
          >
            انصراف
          </button>
          <button
            ...
            className="px-3 py-2 sm:px-4 sm:py-2 bg-pink-600 text-white text-xs sm:text-sm rounded-md hover:bg-pink-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ایجاد بخش
          </button>
        </div>
