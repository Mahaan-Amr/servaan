...
class ChartOfAccountsService {
     ...
     */
    static async initializeIranianChartOfAccounts(tenantId) {
     ...
     */
    static async getAccountHierarchy(tenantId, accountType) {
     ...
     * دریافت حساب با کد
     */
    static async getAccountByCode(tenantId, accountCode) {
     ...
     */
    static async getAccountById(tenantId, id) {
     ...
     * دریافت مانده حساب
     */
    static async getAccountBalance(tenantId, accountId, asOfDate) {
     ...
     * جستجوی حساب‌ها
     */
    static async searchAccounts(tenantId, query, accountType) {
        const whereClause = {
            isActive: true,
            tenantId,
            OR: [
     ...
     * بروزرسانی حساب
     */
    static async updateAccount(tenantId, id, data) {
     ...
     * غیرفعال کردن حساب
     */
    static async deactivateAccount(tenantId, id) {
     ...
     */
    static async getAccountsByType(tenantId, accountType) {
        return await prisma.chartOfAccount.findMany({
            where: {
                accountType,
                isActive: true,
     ...
     */
    static async validateRequiredAccounts(tenantId) {
     ...
     */
    static buildHierarchy(accounts) {
        ...
        function sumBalances(node) {
            if (!node.children || node.children.length === 0) {
                return node.balance || 0;
            }
            // Sum children balances
            const childrenSum = node.children.reduce((sum, child) => sum + sumBalances(child), 0);
     ...
     */
    static getStandardIranianAccounts(tenantId) {
        return [
...
exports.default = ChartOfAccountsService;
