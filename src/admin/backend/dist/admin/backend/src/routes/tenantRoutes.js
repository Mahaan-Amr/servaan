...
router.get('/', authMiddleware_1.authenticateAdmin, async (req, res) => {
    try {
        const { page = 1, limit = 10, search, status, plan, sortBy, sortDir, refresh, businessType, city, country, createdFrom, createdTo, revenueFrom, revenueTo, userCountFrom, userCountTo, hasFeatures } = req.query;
 ...
 * Create a new tenant
 */
router.post('/', authMiddleware_1.authenticateAdmin, (0, authMiddleware_1.requireRole)(['SUPER_ADMIN', 'PLATFORM_ADMIN']), async (req, res) => {
    try {
        const data = req.body;
        // Basic validation
        ...
        const tenant = await TenantService_1.TenantService.createTenant(data);
        await (0, auditLogger_1.auditLog)({
            adminUserId: req.adminUser.id,
...
router.get('/export', authMiddleware_1.authenticateAdmin, async (req, res) => {
    try {
        ...
        const tenantCount = selectedTenants ? selectedTenants.split(',').length : 'all';
        ...
        await (0, auditLogger_1.auditLog)({
            ...
            details: {
                ...
                userCountTo,
                hasFeatures,
                selectedTenants: selectedTenants ? selectedTenants.split(',').length : 'all'
            },
            ipAddress: req.ip || 'unknown'
        });
 ...
 */
router.get('/check-subdomain/:subdomain', authMiddleware_1.authenticateAdmin, async (req, res) => {
    try {
        ...
        const existingTenant = await TenantService_1.TenantService.getTenantBySubdomain(subdomain);
        if (existingTenant) {
            return res.json({
                success: true,
                available: false,
 ...
 */
router.post('/:id/users/reset-password', authMiddleware_1.authenticateAdmin, (0, authMiddleware_1.requireRole)(['SUPER_ADMIN', 'PLATFORM_ADMIN']), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await TenantService_1.TenantService.resetTenantUserPasswordByEmail(id, email, newPassword);
        await (0, auditLogger_1.auditLog)({
            adminUserId: req.adminUser.id,
 ...
 */
router.get('/:id/users', authMiddleware_1.authenticateAdmin, (0, authMiddleware_1.requireRole)(['SUPER_ADMIN', 'PLATFORM_ADMIN']), async (req, res) => {
    try {
        const { id } = req.params;
        const q = req.query['q'] || undefined;
        const users = await TenantService_1.TenantService.listTenantUsers(id, q);
