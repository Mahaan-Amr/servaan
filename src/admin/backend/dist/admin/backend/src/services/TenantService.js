...
class TenantService {
    ...
    static async createTenant(data) {
        ...
        const existing = await prisma_1.prisma.tenant.findUnique({ where: { subdomain: normalizedSubdomain } });
        if (existing) {
    ...
    static async listTenants(params) {
        const { page, limit, search, status, plan, businessType, city, country, createdFrom, createdTo, revenueFrom, revenueTo, userCountFrom, userCountTo, hasFeatures } = params;
        ...
        const revenueByTenant = await withCache(`${params.refresh ? 'noCache:' : ''}monthlyRevenue:${tenantIds.sort().join(',')}:${startOfMonth.toISOString()}`, DEFAULT_TTL_MS, async () => {
            if (tenantIds.length === 0)
                return {};
            const rows = await prisma_1.prisma.orderPayment.groupBy({
                by: ['tenantId'],
                where: {
        ...
        const ordersByTenant = await withCache(`${params.refresh ? 'noCache:' : ''}ordersThisMonth:${tenantIds.sort().join(',')}:${startOfMonth.toISOString()}`, DEFAULT_TTL_MS, async () => {
            if (tenantIds.length === 0)
                return {};
            const rows = await prisma_1.prisma.order.groupBy({
                by: ['tenantId'],
        ...
        let transformedTenants = tenants.map((tenant) => ({
            id: tenant.id,
        ...
        if (revenueFrom !== undefined || revenueTo !== undefined) {
            transformedTenants = transformedTenants.filter((tenant) => {
                const revenue = tenant.monthlyRevenue || 0;
                if (revenueFrom !== undefined && revenue < revenueFrom)
                    return false;
                if (revenueTo !== undefined && revenue > revenueTo)
                    return false;
                return true;
            });
        }
        if (userCountFrom !== undefined || userCountTo !== undefined) {
            transformedTenants = transformedTenants.filter((tenant) => {
                ...
                if (userCountFrom !== undefined && userCount < userCountFrom)
                    return false;
                if (userCountTo !== undefined && userCount > userCountTo)
                    return false;
                return true;
            });
        }
        ...
        if (params.sortBy === 'monthlyRevenue') {
            transformedTenants = transformedTenants.sort((a, b) => (params.sortDir === 'asc' ? 1 : -1) * ((a.monthlyRevenue || 0) - (b.monthlyRevenue || 0)));
        }
        else if (params.sortBy === 'ordersThisMonth') {
            transformedTenants = transformedTenants.sort((a, b) => (params.sortDir === 'asc' ? 1 : -1) * ((a.ordersThisMonth || 0) - (b.ordersThisMonth || 0)));
        }
        return {
    ...
    static async getTenantBySubdomain(subdomain) {
        return await prisma_1.prisma.tenant.findUnique({
    ...
    static async getTenantMetrics(tenantId) {
        ...
        const metrics = await withCache(`tenantMetrics:${tenantId}`, DEFAULT_TTL_MS, async () => {
            const startOfMonth = new Date();
            startOfMonth.setDate(1);
            startOfMonth.setHours(0, 0, 0, 0);
            const [ordersTotal, ordersThisMonth, revenueAgg, revenueMonthAgg, customersTotal, customersVisitedThisMonth] = await Promise.all([
                prisma_1.prisma.order.count({ where: { tenantId } }),
                prisma_1.prisma.order.count({ where: { tenantId, orderDate: { gte: startOfMonth } } }),
                prisma_1.prisma.orderPayment.aggregate({
                ...
                prisma_1.prisma.customer.count({ where: { tenantId, isActive: true } }),
                prisma_1.prisma.customerVisit.count({ where: { tenantId, visitDate: { gte: startOfMonth } } }),
            ]);
     ...
     */
    static async resetTenantUserPasswordByEmail(tenantId, email, newPassword) {
        const user = await prisma_1.prisma.user.findFirst({ where: { tenantId, email } });
        if (!user) {
            throw new Error('TENANT_USER_NOT_FOUND');
        }
        const passwordHash = await bcryptjs_1.default.hash(newPassword, 12);
        const updated = await prisma_1.prisma.user.update({ where: { id: user.id }, data: { password: passwordHash } });
        return { id: updated.id, email: updated.email };
    }
    /**
     * List tenant users (minimal fields)
     */
    static async listTenantUsers(tenantId, search, limit = 50) {
