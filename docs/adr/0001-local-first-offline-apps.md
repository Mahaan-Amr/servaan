...
- Receipt printing is not required for pilot V1 native POS acceptance. After a native sale is queued, the operator sees a Persian success state with local order/payment numbers and pending-sync status. The first Production Desktop Release requires real receipt printing on the approved U80 USB pilot printer, configured by its Windows printer name. Offline/local receipts may print immediately after the local sale and payment are queued, but must include local sale/payment numbers and a small visible pending-sync marker. Receipt print failure must not lose, cancel, delete, or block the queued sale/payment; it is a retryable operator/support problem. Successful offline receipt printing queues the receipt audit flag for sync, and synchronized sales must support a later Canonical Receipt reprint path using canonical backend numbering or verification status.
...
- V1 native Inventory supports only one stock IN or one stock OUT entry per submission, selected from active cached items. Item creation, barcode scanning, bulk receiving, supplier selection, unit conversion, unit price, batch number, expiry date, stock count/audit cycles, and forced reason-code workflows are outside V1.
- V1 native Inventory accepts decimal positive quantities in the selected item's cached canonical unit. Notes are optional for both IN and OUT, with Persian placeholders guiding common reasons.
- V1 native Inventory updates Estimated Local Stock immediately after a local IN/OUT entry is queued. IN increases the estimate, OUT decreases it, and OUT may make the estimate negative. Negative estimates show a Persian warning but do not block the operation; the Canonical Backend later accepts, rejects, or flags the operation during sync.
- V1 native Inventory success shows a Persian confirmation panel with the local document number, item name, movement type, quantity and unit, and pending-sync status. Printing an inventory receipt/document is outside V1.
- Packaged desktop must not expose wrapped web inventory entry/scanner surfaces as the normal V1 inventory path. Desktop inventory navigation opens the native Inventory surface, and desktop attempts to reach old web inventory list/add/remove/scanner routes redirect to `/native?panel=inventory`. Full web inventory pages remain available in the browser/web app for online workflows.
...
- V1 sync status UI shows all unsynced operations grouped by status, because pending operations are still only on the device until the Canonical Backend confirms them. The full list is labeled as unsynced operations, while failed, conflicted, and dependency-waiting operations are labeled as items needing attention. Failed operations show operation type, local number, time, reason, and the shared Sync Now / retry action; conflicts are marked for manager review. Parent/child dependency blocks remain visible instead of being silently hidden. Operator-facing offline, sync, payment, and conflict messages are written in Persian.
...
- Unsynced operations are never auto-deleted. V1 diagnostic exports are operator-safe Redacted Diagnostic Exports: they include device/session/cache context and redacted unsynced-operation summaries, but exclude raw operation payloads, authentication secrets, full local read models, stack traces, and privileged recovery data. Privileged recovery exports may later include encrypted raw queue payloads for manager/admin/support use, but they are a separate workflow.
...
- Desktop/web feature parity is intentionally deferred until after the one-week real-tenant production desktop pilot passes. The first production desktop must prove installer/update/rollback, online login and cache seed, offline restart/unlock, POS sale/payment/receipt/sync, Inventory IN/OUT/sync, support diagnostics, and local-data preservation before broader web workflows are added.
